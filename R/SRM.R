...
SRM.default <- function(
  ...
  fit <- SRM.fit(
    ...
    optim_replications = optim_replications
  )
...
SRM.fit <- function(
  ...
  models_implemented <- c(
    ...
    "CoCAViaR_AS_signs",
    "CoCAViaR_AS_mixed",
    "CoCAViaR_8pCrossCoVaR",
    "CoCAViaR_10p",
    "CoCAViaR_7pVaRViolation",
    "CoCAViaR_Z"
  )

  if (
    !(is.list(model) | (is.character(model) & (model %in% models_implemented)))
  ) {
    stop(
      "Please provide for 'model' either a list of functions or a string matching one of the pre-implemented models!"
    )
  }
  ...
  ### VaR Optimization
  thetav_est_rep <- matrix(
    NA,
    nrow = optim_replications[1],
    ncol = length(theta01)
  )
  ...
  for (rep in 1:optim_replications[1]) {
    ...
    theta01 <- thetav_est_rep[rep, ] +
      MASS::mvrnorm(
        n = 1,
        mu = rep(0, length(theta01)),
        Sigma = 0.1 * diag(length(theta01))
      )
    while (
      !is.finite(mean(
        loss_model_VaR(
  ...
  theta2_est_rep <- matrix(
    NA,
    nrow = optim_replications[2],
    ncol = length(theta02)
  )
  ...
  for (rep in 1:optim_replications[2]) {
    opt_m2 <- optim(
      ...
      fn = function(theta, ...) {
        switch(
          risk_measure,
          MES = {
            mean(loss_model_MES(theta, ...), na.rm = TRUE)
          },
          CoVaR = {
            mean(loss_model_CoVaR(theta, ...), na.rm = TRUE)
          }
        )
      },
      df = data,
    ...
    theta02 <- theta2_est_rep[rep, ] +
      MASS::mvrnorm(
        n = 1,
        mu = rep(0, length(theta02)),
        Sigma = 0.1 * diag(length(theta02))
      )
    while (
      !is.finite(switch(
        risk_measure,
  ...
  data <- data %>%
    dplyr::mutate(
      VaR = as.numeric(m_est$m1),
      risk_measure := as.numeric(m_est$m2)
    ) %>%
...
summary.SRM <- function(SRM_object, method = 'asymptotic', ...) {
  ...
  coef_mat <- cbind(
    ...
    `Pr(>|t|)` = 2 *
      stats::pt(
        abs(tval),
        dim(SRM_object$data)[1] - length(SRM_object$theta),
        lower.tail = FALSE
      )
  )
  ...
  rownames(coef_mat_risk_measure) <- SRM_object$colnames[[
    SRM_object$risk_measure
  ]]
...
print.summary.SRM <- function(sum.SRM_object) {
  ...
  cat("\nVaR Coefficients:\n")
  stats::printCoefmat(sum.SRM_object$coef_mat_VaR, signif.legend = FALSE)
  # cat("\nCoVaR Coefficients:\n")
...
print.SRM <- function(obj, digits = 4) {
  theta_info <- theta_fun(
    model = obj$model,
    theta = obj$theta,
    df = obj$data %>% dplyr::select(-c("VaR", "risk_measure"))
  )
...
autoplot.SRM <- function(obj, facet_names = NULL) {
  if (is.null(facet_names)) {
    if (obj$risk_measure == "CoVaR") {
      facet_names <- c("X / VaR", "Y / CoVaR")
    } else {
      facet_names <- c("X / VaR", "Y / MES")
    }
  }
  ...
  df_long <- dplyr::left_join(
    df_tmp %>%
      ...
      reshape2::melt(
        id.vars = c("Date", "VaR_violation"),
        variable.name = "Symbol",
        value.name = "NegativeReturns"
      ),
    df_tmp %>%
      ...
      reshape2::melt(
        id.vars = c("Date", "VaR_violation"),
        variable.name = "Symbol",
        value.name = "risk_measureForecasts"
      ),
...
forecast.SRM <- function(obj, newdata = NULL) {
  ...
  m <- model_fun(
    obj$theta,
    df = df,
    prob_level = obj$prob_level,
    model = obj$model,
    risk_measure = obj$risk_measure,
    forecast = TRUE
  )
...
PIforecast.SRM <- function(obj, level = 0.9, newdata = NULL) {
  ...
  PIforecast <- list(
    m1_FC = c(tail(m_low$m1, 1), tail(m_up$m1, 1)),
